...
namespace AppVMC.ViewModels
{
    public class AfiliadoViewModel : ViewModelBase
    {
        ...
        {
            esteAfiliado = ServiceAfiliado.GetNew().Result;
            esteAfiliado.Id = afiliadoList.Count == 0 ? 1 : afiliadoList.Count + 1;
            await Task.Run(()=>Dialog.Show<AfiliadoNovo>("Novo Afiliado"));
        }
        ...
        protected async Task NovaMensalidade()
        {
            ...
            await Task.Run(()=>Dialog.Show<GerarMesalidade>("Nova Mensalidade"));
        }
        ...
        {
            isEditing = true;
            esteAfiliado =await Task.FromResult( afiliadoList.Find(f => f.Id == id));
